    ...
    with open(f'{filename}.csv', 'r') as f:
        ...
        for row in reader:
            ...
            if len(code) == 1:
                ...
                city, ccreated = Kota.objects.get_or_create(name=row[1], code=row[0].replace(".", ""), provinsi=provinsi)
                print(f"Kota: {city}, Created: {ccreated}")
            elif len(code) == 3:
                city = Kota.objects.get(code="".join(code[:-1]))
                kecamatan, kcreated = Kecamatan.objects.get_or_create(name=row[1], code=row[0].replace(".", ""), kota=city)
                print(f"Kecamatan: {kecamatan}, Created: {kcreated}")
            elif len(code) == 4:
                kecamatan = Kecamatan.objects.filter(code="".join(code[:-1])).first()
                kelurahan, klcreated = Kelurahan.objects.get_or_create(name=row[1], code=row[0].replace(".", ""), kecamatan=kecamatan)
